...
public class HomogeniousRecordWriteReadDecimalBenchmark extends Benchmark {
    ...
    static RingBuffers ringBuffers= RingBuffers.buildNoFanRingBuffers(new RingBuffer((byte)testCatalog.clientConfig().getPrimaryRingBits(),(byte)testCatalog.clientConfig().getTextRingBits(),testCatalog.ringByteConstants(), testCatalog.getFROM()));
    ...
    protected int staticWriteReadOverheadGroup(int reps) {
        ...
        for (int i = 0; i < reps; i++) {
            ...
            staticReader.closeGroup(groupToken | (OperatorMask.Group_Bit_Close << TokenBuilder.SHIFT_OPER), idx, reader, rbRingBuffer, 1==rbRingBuffer.consumerData.from.addByteCountToFragment[0]);
        }
        return result;
    }
    ...
    protected long staticWriteReadDecimalGroup(int reps, int token, int groupToken, int pmapSize) {
        ...
        for (int i = 0; i < reps; i++) {
            ...
            int idx = TokenBuilder.MAX_INSTANCE & groupToken;
            staticReader.closeGroup(groupToken | (OperatorMask.Group_Bit_Close << TokenBuilder.SHIFT_OPER), idx, reader, rbRingBuffer, FieldReferenceOffsetManager.USE_VAR_COUNT && 1==rbRingBuffer.consumerData.from.addByteCountToFragment[0]);
        }
        return result;
    }
