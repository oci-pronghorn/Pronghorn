...
public class MetaMessageWriter  {
    ...
    private static void writeNullableMessage(RingBuffer ring, int msg, boolean isNullable, String columnName, Object value) {
        if (isNullable) {
            RingBuffer.blockWriteMessage(ring, msg);
            if (columnName != null) {
                RingBuffer.validateVarLength(ring, columnName.length());
				int sourceLen = columnName.length();
				final int p = RingBuffer.copyASCIIToBytes(columnName, 0, sourceLen, ring); 
				RingBuffer.addBytePosAndLen(ring, p, sourceLen);
            }
            RingBuffer.setValue(ring.structuredLayoutRingBuffer,ring.mask,RingBuffer.getWorkingHeadPositionObject(ring).value++,(value != null) ? 1 : 0); // true if not null
            RingBuffer.publishWrites(ring);
        }
    }

    private static void writeNullableIntMessage(RingBuffer ring, boolean isNullable, boolean isSigned, String columnName, Object value) {
    ...
    private static void writeIntMessage(RingBuffer ring, boolean isNullable, boolean isSigned, String columnName, int value) {
        ...
        RingBuffer.setValue(ring.structuredLayoutRingBuffer,ring.mask,RingBuffer.getWorkingHeadPositionObject(ring).value++,value);
        RingBuffer.publishWrites(ring);
    }

    public static void writeByteMessage(RingBuffer ring, boolean isNullable, boolean isSigned, String columnName, Object value) {
    ...
    public static void writeShortMessage(RingBuffer ring, boolean isNullable, boolean isSigned, String columnName, Object value) {
    ...
    public static void writeIntMessage(RingBuffer ring, boolean isNullable, boolean isSigned, String columnName, Object value) {
        // int is written as int32/uint32
        writeNullableIntMessage(ring, isNullable, isSigned, columnName, value);
        if (value != null)
    ...
    public static void writeBooleanMessage(RingBuffer ring, boolean isNullable, String columnName, Object value) {
        ...
        if (value != null) {
        	RingBuffer.blockWriteMessage(ring, (columnName == null) ? MetaMessageDefs.MSG_BOOLEAN_LOC : MetaMessageDefs.MSG_NAMEDBOOLEAN_LOC);
            if (columnName != null) {
                RingBuffer.validateVarLength(ring, columnName.length());
				int sourceLen = columnName.length();
				final int p = RingBuffer.copyASCIIToBytes(columnName, 0, sourceLen, ring); 
            ...
            RingBuffer.setValue(ring.structuredLayoutRingBuffer,ring.mask,RingBuffer.getWorkingHeadPositionObject(ring).value++,((Boolean) value).booleanValue() ? 1 : 0);
            RingBuffer.publishWrites(ring);
        }
    }


    public static void writeLongMessage(RingBuffer ring, boolean isNullable, boolean isSigned, String columnName, Object value) {
        ...
        if (value != null) {
            ...
            RingBuffer.addLongValue(ring.structuredLayoutRingBuffer, ring.mask, RingBuffer.getWorkingHeadPositionObject(ring), ((Number)value).longValue());
            RingBuffer.publishWrites(ring);
        }
    }

    public static void writeDecimalMessage(RingBuffer ring, boolean isNullable, String columnName, Object value) {
        // long is written as int64/uint64
        ...
        if (value != null) {
            ...
            RingBuffer.addDecimal(bd.scale(), bd.unscaledValue().longValue(), ring);
            RingBuffer.publishWrites(ring);
        }
    }


    public static void writeDoubleMessage(RingBuffer ring, boolean isNullable, String columnName, Object value) {
        ...
        if (value != null) {
            ...
            RingBuffer.addLongValue(ring.structuredLayoutRingBuffer, ring.mask, RingBuffer.getWorkingHeadPositionObject(ring), bits);
            RingBuffer.publishWrites(ring);
        }
    }


    public static void writeFloatMessage(RingBuffer ring, boolean isNullable, String columnName, Object value) {
        // double is written as int64
        ...
        if (value != null) {
            ...
            RingBuffer.setValue(ring.structuredLayoutRingBuffer,ring.mask,RingBuffer.getWorkingHeadPositionObject(ring).value++,bits);
            RingBuffer.publishWrites(ring);
        }
    }
    ...
    public static void writeDateTimeMessage(RingBuffer ring, boolean isNullable, String columnName, Object value) {
        ...
        if (value != null) {
            ...
            long millisecondsSinceEpoch = ((java.util.Date) value).getTime(); // java.sql.Date, java.sql.Time,
                                                                              // java.sql.Timestamp
            RingBuffer.addLongValue(ring.structuredLayoutRingBuffer, ring.mask, RingBuffer.getWorkingHeadPositionObject(ring), millisecondsSinceEpoch);
            RingBuffer.publishWrites(ring);
        }
    }
    
    public static void writeTimestampMessage(RingBuffer ring, boolean isNullable, String columnName, java.sql.Timestamp timestamp, int tzOffset) {
        ...
        if (timestamp != null) {
            ...
            RingBuffer.addLongValue(ring.structuredLayoutRingBuffer, ring.mask, RingBuffer.getWorkingHeadPositionObject(ring), millisecondsSinceEpoch);
            RingBuffer.setValue(ring.structuredLayoutRingBuffer,ring.mask,RingBuffer.getWorkingHeadPositionObject(ring).value++,timestamp.getNanos());
            RingBuffer.setValue(ring.structuredLayoutRingBuffer,ring.mask,RingBuffer.getWorkingHeadPositionObject(ring).value++,tzOffset);
            RingBuffer.publishWrites(ring);
        }
    }

    public static void writeByteArrayMessage(RingBuffer ring, boolean isNullable, String columnName, Object value) {
    ...
    public static void writeSerializedMessage(RingBuffer ring, boolean isNullable, String columnName, Object value) throws IOException {
        ...
        if (value != null) {
            ...
            RingBuffer.addByteArray(bytes, 0, bytes.length, ring);
            RingBuffer.publishWrites(ring);
        }
    }

    public static void writeBeginGroupMessage(RingBuffer ring, String name) {
        int msg = (name == null) ? MetaMessageDefs.MSG_BEGINGROUP_LOC : MetaMessageDefs.MSG_NAMEDBEGINGROUP_LOC;
        RingBuffer.blockWriteMessage(ring, msg);
        if (name != null) {
            RingBuffer.validateVarLength(ring, name.length());
			int sourceLen = name.length();
			final int p = RingBuffer.copyASCIIToBytes(name, 0, sourceLen, ring); 
			RingBuffer.addBytePosAndLen(ring, p, sourceLen);
        }
        RingBuffer.publishWrites(ring);
    }

    public static void writeEndGroupMessage(RingBuffer ring) {
    	RingBuffer.blockWriteMessage(ring, MetaMessageDefs.MSG_ENDGROUP_LOC);
        RingBuffer.publishWrites(ring);
    }

    public static void writeASCIIMessage(RingBuffer ring, boolean isNullable, String columnName, Object value) {
        ...
        if (value != null) {
			...
			final int p = RingBuffer.copyASCIIToBytes(source, 0, sourceLen, ring); 
			RingBuffer.addBytePosAndLen(ring, p, sourceLen);
            RingBuffer.publishWrites(ring);
        }
    }
    
    public static void writeUTF8Message(RingBuffer ring, boolean isNullable, String columnName, Object value) {
        ...
        if (value != null) {
			...
			CharSequence source = (String) value;
            RingBuffer.validateVarLength(ring, source.length()<<3);//UTF8 encoded bytes are longer than the char count (6 is the max but math for 8 is cheaper)
			RingBuffer.addBytePosAndLen(ring, RingBuffer.bytesWorkingHeadPosition(ring), RingBuffer.copyUTF8ToByte(source, source.length(), ring));
            RingBuffer.publishWrites(ring);
        }
    }
