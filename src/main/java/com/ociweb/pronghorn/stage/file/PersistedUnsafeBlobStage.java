...
@Deprecated //does not encrypt storage.
public class PersistedUnsafeBlobStage extends PronghornStage {

	private static final Logger logger = LoggerFactory.getLogger(PersistedUnsafeBlobStage.class);
	...
	public static PersistedUnsafeBlobStage newInstance(GraphManager graphManager, 
	            ...
	            File rootFolder) {
		return new PersistedUnsafeBlobStage(graphManager,storeRequests,loadResponses,fileSizeMultiplier,maxIdValueBits,rootFolder);
	}
	
	public PersistedUnsafeBlobStage(GraphManager graphManager, 
			                     Pipe<PersistedBlobStoreSchema> storeRequests,  //load request
			                     Pipe<PersistedBlobLoadSchema> loadResponses,  //Write ack, load done?
	...
	public void startup() {
		...
		/////////////
		try {			
			blobDataFile[activeFile].seek(blobDataFile[activeFile].length());//find end of active file.
