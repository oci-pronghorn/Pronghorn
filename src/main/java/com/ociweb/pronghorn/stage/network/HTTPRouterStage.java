...
public class HTTPRouterStage<T extends Enum<T> & HTTPContentType,
  ...
  //TODO: double check that this all works iwth ipv6.
    ...
    //TODO: AA add an additional output pipe so the router can relay the input pipe to an output log
    ...
    public HTTPRouterStage(GraphManager gm, Pool<Pipe<ServerRequestSchema>> input, Pipe<HTTPRequestSchema>[] outputs, Pipe errorPipe,
                           CharSequence[] paths, long[] headers, int[] messageIds, //TODO: remove message IDS not needed any longer
    ...
    public void startup() {
        ...
        urlMap = new TrieParser(1024,false);  //TODO: build size needed for these URLs dynamically 
        ...
        //for unknowns just support -1 because its faster and we need not capture the  unknown value.
...
private boolean route(TrieParserReader trieReader, long channel, int idx) {    
    ...
    if (Pipe.hasRoomForWrite(staticRequestPipe) ) {
        ...
        //TODO: review this to determine better way to keep the offset locations.
...
private boolean hasNoActiveChannel(int idx) {
    ...
    private int parseHeaderFields(final int routeId, Pipe<HTTPRequestSchema> staticRequestPipe, int revisionId, boolean debugMode) {
        ...
        //if pipe can not make use of any headers abandon the rest of the data.
        ...
        //output write message needs:
        //  channel
        //  payload
        //  close on xmit boolean, change to pipe n, other things??
      ...
      //  System.out.println("look in "+headerMap);
        int headerId = 0;
        int iteration = 0;
        boolean isDone = false;
        while (-101!=headerId && TrieParserReader.parseHasContent(trieReader)){
            
            //staticRequestPipe.validateVarLength(rb, length)
            //TODO: what is the max length for a header field, how to stop this from running to end of data ?
            
            //return -101 upon end of header??
            headerId = (int)TrieParserReader.parseNext(trieReader, headerMap);
            //TODO: must guarntee this never hangs here on loop. upon -1 must move on??
            ...
            if (END_OF_HEADER_ID== headerId) {
                if (iteration==0) {
                    throw new RuntimeException("should not have found end of header");
                     ...
                     isDone=true;
                    // System.out.println("found end "+headerId);
            ...
            if (-1 == headerId) {
                if (debugMode) {
                    System.out.println("exit a "+headerId);
                }
                //nothing valid was found so this is incomplete.
                return ServerConnectionWriterStage.INCOMPLETE_RESPONSE_MASK; 
            }
//            else {
//                H h = supportedheaderRequests.getEnumConstants()[headerId];
//              
//               System.out.println(h+" "+headerId+ "  "+SequentialTrieParserReader.capturedFieldBytesAsUTF8(trieReader, 0, new StringBuilder()));
//           
//            }
            ...
            if (headerIdUpgrade == headerId) {
                ...
                //NOTE: for now do not support upgrade just print the request
                
               // System.out.print("upgrade :");
                //System.out.println(TrieParserReader.capturedFieldBytesAsUTF8(trieReader, 0, new StringBuilder()) );
               
                
            } else if (headerIdContentLength == headerId) {
                //read the length, a long
...
//            int len = TrieParserReader.capturedFieldBytes(trieReader, 0, data, 0, 127);
//            System.out.println("parsed "+headerId+"  "+HTTPHeaderRequestKeyDefaults.values()[headerId]+"  "+new String(data, 0, len));
//            
            ...
            if (0 == (maskId&headerMask) ) {
              ...
              // System.err.println("found a target header for 2 XXXX");
                
                //clear this bit to mark it as seen
                headerMask = headerMask^maskId;
                //copy the caputred bytes to the specific offset, eg known position for this field.
                TrieParserReader.writeCapturedValuesToPipe(trieReader, staticRequestPipe, basePos+offsets[headerId]);
           
            }      
           // System.out.println(headerMask+" id "+headerId);
            iteration++;
        }
        if (!isDone) {
            if (debugMode) {
