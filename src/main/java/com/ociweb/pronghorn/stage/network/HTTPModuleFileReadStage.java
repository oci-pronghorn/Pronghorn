...
public class HTTPModuleFileReadStage<   T extends Enum<T> & HTTPContentType,
    ...
    private int runningBlobPosition;
    private int originalBlobPosition;
...
//  </html>
    ...
    static < T extends Enum<T> & HTTPContentType> IntHashTable buildFileExtHashTable(Class<T> supportedHTTPContentTypes) {
        int hashBits = 13; //8K
    ...
    public void run() {
        ...
        try {
            ...
            disconnectDueToError(activeReadMessage, output, ioex);
        }
   ...
   //     System.out.println((null==activeFileChannel) + " &&  "+Pipe.hasContentToRead(input)+ " && "+ Pipe.hasRoomForWrite(output));
        ...
        while (null==activeFileChannel && Pipe.hasContentToRead(input) && Pipe.hasRoomForWrite(output)) {
            ...
            if (msgIdx == HTTPRequestSchema.MSG_FILEREQUEST_200) {
                activeReadMessage = Pipe.sizeOf(input, msgIdx);
    ...
    private void beginReadingNextRequest() {
        ...
        int bytesPosition = Pipe.bytePosition(meta, input, bytesLength); //TODO this is returning an odd value, is the base offset right, can we add test for this?
        ...
        activeRequestContext = Pipe.takeValue(input) | ServerConnectionWriterStage.END_RESPONSE_MASK;
    ...
    private int setupUnseenFile(TrieParser trie, final int bytesLength, final byte[] bytesBackingArray,
        ...
        try {
            ...
            System.out.println();
            System.out.println("len:"+bytesLength);
            int p = bytesPosition;
            int l = bytesLength;
            while (--l >= 0) {
                System.out.println("_ "+(bytesMask&p )+" ^ "+((char)bytesBackingArray[bytesMask & p]  )+" | "+Integer.toString(bytesBackingArray[bytesMask & p++])+"   "+p+"  "+bytesMask);
            }
            
            
            new Exception("Should not be called  path was "+bytesLength+"   '"+pathString+"'").printStackTrace(); //TODO: first one is broken
    ...
    private void beginSendingFile(int httpRevision, int requestContext, int pathId, int verb, int sequence) {
        try {                                               
            ...
            int requestContext1 = VERB_GET==verb ? 0 : requestContext;
            int status = 200;
            
            
            totalBytes += publishHeaderMessage(requestContext, sequence, requestContext1, status, writer,
                                                   output, activeChannelHigh, activeChannelLow, etagBytes[pathId], httpSpec.revisionBytes[httpRevision], httpSpec.contentTypeBytes[type[pathId]], fileSizeAsBytes[pathId]); 
            
            runningBlobPosition = originalBlobPosition = Pipe.getBlobWorkingHeadPosition(output);
            PipeHashTable.setLowerBounds(outputHash, 1 + originalBlobPosition - output.sizeOfBlobRing );
    ...
    private void publishErrorHeader(int httpRevision, int requestContext, int pathId, int sequence, Exception e) {
        ...
        publishError(requestContext, sequence, errorStatus, writer, output, activeChannelHigh, activeChannelLow,
                httpSpec.revisionBytes[httpRevision], httpSpec.contentTypeBytes[type[pathId]]);
        
        Pipe.confirmLowLevelRead(input, activeReadMessage);
    ...
    private void writeBodiesWhileRoom(int channelHigh, int channelLow, int sequence, Pipe<ServerResponseSchema> localOutput, FileChannel localFileChannel, int pathId) throws IOException {
       if (null != localFileChannel) {
         ...
         int localBlobPosition = runningBlobPosition;
         while (Pipe.hasRoomForWrite(localOutput)) {
            ...
            if (blobPosition>0) {
                ...
                Pipe.copyBytesFromToRing(Pipe.blob(localOutput), Pipe.safeBlobPosAdd(blobPosition,localPos), Pipe.blobMask(localOutput), 
                                         Pipe.blob(localOutput), Pipe.safeBlobPosAdd(originalBlobPosition,localPos), Pipe.blobMask(localOutput), len);
                
                publishBodyPart(channelHigh, channelLow, sequence, localOutput, localBlobPosition, len);   
                localPos += len;
                localBlobPosition += len;
            } else {
            
                int len;
                if ((len=localFileChannel.read(Pipe.wrappedBlobForWritingA(localBlobPosition, localOutput), localPos)) >= 0) {
                    
                    publishBodyPart(channelHigh, channelLow, sequence, localOutput, localBlobPosition, len);   
                    localPos += len;
                    localBlobPosition += len;
                } else {
                    System.out.println("ERROR: this block should not have been called.");
                    Pipe.confirmLowLevelRead(input, activeReadMessage);
                    ...
                    PipeHashTable.replaceItem(outputHash, fcId[pathId], originalBlobPosition);
    ...
    private void publishBodyPart(int channelHigh, int channelLow, int sequence, Pipe<ServerResponseSchema> localOutput, int originalBlobPosition, int len) {
