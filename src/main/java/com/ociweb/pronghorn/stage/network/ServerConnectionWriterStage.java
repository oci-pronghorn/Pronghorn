...
public class ServerConnectionWriterStage extends PronghornStage {
    ...
    private ByteBuffer                         writeABuff;
    private ByteBuffer                         writeBBuff;
    ...
    public void startup() {
        ...
        try {
            coordinator.registerSelector(pipeIdx, Selector.open());
        } catch (IOException e) {
        ...
        while (done && --c>= 0) {
            while (done && Pipe.hasContentToRead(dataToSend[activePipe])) {
                ...
                if (peekId>=0) {
                    ...
                    boolean isBlocked = dataToSend.length>1 && sequenceNo!=expected;
                    if (isBlocked) {
                        nextPipe();
                        continue;
                    }
                    expectedSquenceNos[pos] = expected+1;
    ...
    private void loadPayloadForXmit() {
        ...
        writeABuff = Pipe.wrappedBlobReadingRingA(pipe, meta, len);
        writeBBuff = Pipe.wrappedBlobReadingRingB(pipe, meta, len);
        writeDone = false;
    ...
    private boolean writeToChannel(SocketChannel channel) {
        ...
        try {                
            channel.write(writeABuff);
            if (0==writeABuff.remaining()) {
                channel.write(writeABuff);
                if (0==writeBBuff.remaining()) {
                    markDoneAndRelease(); 
