...
public class Appendables {
    ...
    public static <A extends Appendable> A appendArray(A target, char left, byte[] b, char right) throws IOException {
        return appendArray(target, left, b, right, b.length);
    }
    
    
    public static <A extends Appendable> A appendArray(A target, char left, byte[] b, char right, int bLength) throws IOException {
        if (b != null) {        
            int iMax = bLength - 1;
            if (iMax == -1) {
                target.append(left).append(right);
