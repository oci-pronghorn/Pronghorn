...
public class InverseQuantizer {
	...
	public static void dequantize(ArrayList<MCU> mcus, Header header) {
		for (int i = 0; i < mcus.size(); ++i) {
			dequantizeMCU(mcus.get(i).yAc, header.quantizationTables.get(header.colorComponents.get(0).quantizationTableID));
			dequantizeMCU(mcus.get(i).cbAc, header.quantizationTables.get(header.colorComponents.get(1).quantizationTableID));
			dequantizeMCU(mcus.get(i).crAc, header.quantizationTables.get(header.colorComponents.get(2).quantizationTableID));
		}
		return;
	}
}
