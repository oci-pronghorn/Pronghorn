...
public class SocketIOStageTest {
	...
	public class EchoTestServer extends PronghornStage {
		...
		public EchoTestServer(GraphManager gm, boolean isClient, ServerCoordinator coordinator, 
				           Pipe<NetPayloadSchema>[] inputs, Pipe<NetPayloadSchema>[] outputs, Pipe<ReleaseSchema>... release) {
			super(gm, inputs, join(outputs,release));
			this.inputs = inputs;
			this.outputs = outputs;
			this.openCount = inputs.length;
	...
	public void testRoundTrip() {
		...
		NetGraphBuilder.buildServerGraph(gm, coordinator, new ServerFactory() {
			...
			//TODO: is this a mistake? seems like EchoTest must be server side using..
			...
			public void buildServer(GraphManager graphManager, ServerCoordinator coordinator,
				...
				new EchoTestServer(graphManager, false, coordinator, receivedFromNet, sendingToNet, releaseAfterParse);
